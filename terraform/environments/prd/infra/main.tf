...
terraform {
  backend "gcs" {
    ...
    prefix = "pd/p-601-pd-bcleadsmgmt-prd"
  }
}
...
module "cloud_sql_instance" {
  ...
  service_account_iam = var.service_account_email
...
module "network_attachement" {
  ...
  subnet_name           = "gcp-snt-pd-usc1-601-bcleadsmgmt-servicenow-psc-pd"
  network_attachment_name = "gcp-nat-pd-usc1-601-bcleadsmgmt-servicenow-psc-pd"
}