...
def write_lead_data_to_db(batch_id, storage_config, database_config, transform_config:TransformConfig,data_load_type="initial"):
    ...
    if data_load_type == 'initial':
        ...
        ba_util.update_batch_id(batch_id, 'lead', "db_load", len(lead_df), len(lead_df), "Completed", database_config)
        ...
        try:
            ...
            ba_util.update_batch_id(batch_id, 'lead', "db_load", lead_success, lead_failure, "Completed", database_config)
        except Exception as ex:
            print("ERROR: load to database failed")
            print(ex)
            ba_util.update_batch_id(batch_id, "lead", "db_load", len(cleansed_lead_df), 0,
...
def process_contact(batch_id, storage_config, database_config, data_load_type="initial"):
    ...
    try:
        ...
        print("ERROR: load to database failed")
        print(ex)
        ba_util.update_batch_id(batch_id, "lead", "db_load", 0, 0,
                        "Failed", database_config)
    ...
    try:
        if data_load_type == "initial":
            ...
            success_count, failure_count = upsert_using_primary_key(transformed_df,
                                                                    ...
                                                                    "pos_id",
                                                                    database_config)
            ba_util.update_batch_id(batch_id, "pos", "db_load", success_count, failure_count,
                            "Completed", database_config)
        ...
        print(ex)
        ba_util.update_batch_id(batch_id, "pos", "db_load", 0, 0,
                        "Failed", database_config)
...
def get_record_snow_to_gcs(batch_id,data_type:str, snow_config: SnowConfig, gcs_config: StorageConfig,
                           database_config: DatabaseDetail):
    if not batch_id:
        batch_id = uuid.uuid4()
...
def get_data_from_snow(batch_id, config: JobConfig, ):
    ...
    get_record_snow_to_gcs(batch_id, "pos", job_config.snow_config, job_config.storage_config,
                           job_config.db_config)
    return batch_id
...
def load_data_to_db(batch_id, config: JobConfig):
    ...
    if not batch_id:
        batch_id = uuid.uuid4()
    write_lead_data_to_db(batch_id, config.storage_config, config.db_config,config.transform_config,config.data_load_type)
    write_pos_data_to_db(batch_id, config.storage_config, config.db_config,config.transform_config, config.data_load_type)
