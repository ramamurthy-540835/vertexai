...
from costco.leadmgmt.config.Configuration import JobConfig, StorageConfig, SnowConfig
...
def dedup_account(cleansed_df):
    ...
    de_duped_df = merged.drop(columns=['account_id_new'])
...
def write_lead_data_to_db(batch_id, storage_config, database_config, data_load_type="initial"):
    ...
    delta_data_type = {'lead_number': str}
...
def write_pos_data_to_db(batch_id, storage_config: StorageConfig, database_config: DatabaseDetail,
    ...
    data_type_dict = {"u_number": str, "u_lead_matched": str,
                      "u_customer_id": "Int64", "u_business_name": str, "u_membership_number": "Int64",
                      "u_phone_number": str, "u_warehouse_number": 'Int64', "u_sic4": "Int64", "u_sic6": "Int64",
                      "u_zip_code": str, "u_bd_industry": str, "u_address_line_1": str, "u_address_line_2": str,
                      "u_city": str, "u_customer_name": str, "u_email": str, "u_first": str, "u_last": str,
                      "u_order_number": 'Int64', "u_order_amount": 'float', "u_order_date": str, "u_shop_type": "str"}
    pos_df = read_gcs_to_dataframe(storage_config.input_bucket_name,
                                   f"{storage_config.pos_input_folder}/", datatype=data_type_dict,
                                   encoding="ISO8859-1")

    rename_dict = {"u_number": "pos_id", "u_lead_matched": "lead_id",
                   "u_customer_id": "customer_id", "u_business_name": "business_name",
                   "u_membership_number": "membership_number",
                   "u_phone_number": "phone", "u_warehouse_number": "warehouse_number", "u_sic4": "sic4_code",
                   "u_sic6": "sic6_code",
                   "u_zip_code": "zip_code", "u_bd_industry": "bd_industry", "u_address_line_1": 'address_line_one',
                   "u_address_line_2": 'address_line_two',
                   "u_city": "city", "u_customer_name": "customer_name", "u_email": "email", "u_first": "first_name",
                   "u_last": "last_name",
                   "u_order_number": 'order_number', "u_order_amount": 'order_amount', "u_order_date": "order_date",
                   "u_shop_type": "shop_type"}
    transformed_df = transform_pos(pos_df, rename_dict)
    ls_pos_columns = ['pos_id', "lead_id", 'order_number', 'order_date', 'order_amount', 'warehouse_number',
                      'business_name', 'sic4_code', 'sic6_code', 'bd_industry', 'phone',
                      'address_line_one', 'address_line_two', 'city', 'zip_code', 'shop_type', 'membership_number']
...
if __name__ == "__main__":
    ...
    if stage.lower() == "snow_to_gcs":
        ...
        try:
            batch_id = uuid.uuid4()
            write_lead_data_to_db(batch_id, job_config.storage_config, job_config.db_config, job_config.data_load_type)
        except Exception as ex:
        ...
        try:
            batch_id = uuid.uuid4()
            write_pos_data_to_db(batch_id, job_config.storage_config, job_config.db_config,
